...
class NewAssetAdmin(admin.ModelAdmin):
    ...
    actions = ['approved_selected_new_assets']

    def approved_selected_new_assets(self, request, queryset):
        selected = request.POST.getlist(admin.ACTION_CHECKBOX_NAME)
        success_upline_number = 0
        for asset_id in selected:
            obj = handler.ApproveAsset(request, asset_id)
            ret = obj.asset_upline()
            if ret:
                success_upline_number += 1

        self.message_user(request, "成功批准 %s 条新资产上线!" % success_upline_number)
    approved_selected_new_assets.short_description = "批准选择的新资产"
